...
const changeToLower = zooAnimals.map((name) => {
...
const filterPopulations = zooAnimals.filter((animal, index) => {
  if (animal.population < 5) {
    lowPopulationAnimals.push(zooAnimals[index]);
  }
});
...
*/
let populationTotal = zooAnimals.reduce((population, currentAnimal) => {
  return population + currentAnimal.population;
}, 0);
