...
const displayNames = [];
  zooAnimals.forEach(function(element){
    ...
    let display = "Name: " + element.animal_name + ", " + "Scientific: " + element.scientific_name
...
*/
function populationTotal(data){
  let populationTotal = data.reduce((accumulator, item) => accumulator + item) = 0;
}
...
function consume(a, b){
  let consumeAB = b.map(function(item, index){
    return item, a[index];
  })
  return consumeAB;
}
