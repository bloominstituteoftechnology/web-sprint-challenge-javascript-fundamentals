...
//because both nested function and internal are both inside the myFunction function
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    ...
    arr.forEach((item,item2) => {
      nameArr.push(`name: ${arr[item2].animal_name}, scientific: ${arr[item2].scientific_name}`)
    })
    return nameArr
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    arr.filter((item) => item.population < 5)
  }
  ...
  */
  ...
  function USApop(arr){
    let totalPop = arr.reduce((accumulator,item)=>{
      return item.population + accumulator
    },0)
    return totalPop
  }
