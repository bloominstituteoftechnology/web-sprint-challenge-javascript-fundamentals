...
// nestedFunction can access internal because it is being declared within the same myFunction. and it is passed into nestedFunction as a parameter
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    let nameArray = [];
    arr.forEach((item, item2)=>{
      nameArray.push(`name: ${arr[item2].animal_name}, scientific: ${arr[item2].scientific_name}`)
    })
    return nameArray;
  }
  ...
  */
  ...
  function lowerCaseNames(arr){
    let caseSensitive = arr.map((item, item2)=>{
      return arr[item2].animal_name.toLowerCase();
    })
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    let popCount = arr.filter((item, item2)=>{
      return arr[item2].population < 5;
    })
    return popCount;
  }
  ...
  */
  ...
  function USApop(arr){
    let totalPop = arr.reduce((accumulator, item2)=>{
      return item2.population + accumulator
    }, 0)
    return totalPop
  }
