...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// myfunction gets ran  and when so it runs a console of the const "external" 
// then the next line of code to be ran is the "nestedFunction" which is notin the global scope but in the same scope as its parent function



/* 🚀🚀🚀 Task 2: Counter 🚀🚀🚀 */
/* Create a function called `summation` that 
accepts a parameter and uses a counter to 
...
const zooAnimals = [
  ...
  Populate the displayNames array with only 
  the animal_name and scientific_name of each animal. 
 ...
 function lowerCaseNames(array){
  ...
  create a new array of objects called lowPopulationAnimals 
  which contains only the animals with a population less than 5.
*/

  const lowPopulationAni = [];
  function lowPopulationAnimals(array3){
    zooAnimals.filter(function(item){
      if(item.population <= 5){
        lowPopulationAni.push(item)
      }
    })
    return lowPopulationAni
  }
...
Remember the reduce method takes two arguments: a callback (which itself takes two args),
  ...
  */
  function USApop(array){
    const totalPop= array.reduce(function(counter,item){
...
//  for a cuboid's volume or surface area. 
