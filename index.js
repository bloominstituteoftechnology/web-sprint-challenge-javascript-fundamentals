...
console.log(lowercase);
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const lowpop = arr.filter((a) => {return a.population < 5;})
    console.log(lowpop);
   }
  ...
  */
  ...
  function USApop(arr){
    const thing = arr.map((a) => {return a.population;})
    const otherthing = thing.reduce((a, b) => a + b, 0);
...
/* 🐴🐴🐴 Step 3: Surface Area Method 🐴🐴🐴
  ...
  CuboidMaker.prototype.surfaceArea = function(){
    let area = (this.length * this.width) + (this.length * this.height) + (this.width * this.height);
     console.log(area)
   }
