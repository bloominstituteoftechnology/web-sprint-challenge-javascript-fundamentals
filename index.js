...
//Because both the nested function and the internal variable are inside the function myFunction. Both are in scope inside myFunction
...
const zooAnimals = [
  ...
  */
  ...
  function lowPopulationAnimals(list){
    return list.filter(i => i.population < 5);
  }
  ...
  */
  ...
  function USApop(list){
    return list.reduce((total, item) => total + item.population, 0)
  }
  USApop(zooAnimals)
...
*/
function CuboidMaker(deets){
  this.length = deets.length;
  this.width = deets.width;
  this.height = deets.height;
}
